...
import { buildProductsEndpoint, getNextSkip, normalizeQueryValue } from "@/features/products/utils";
...
describe("products utils", () => {
  ...
  it("builds search endpoint when search exists", () => {
    expect(buildProductsEndpoint({ limit: 12, skip: 0, search: "iphone" })).toBe("/products/search");
  });

  it("builds category endpoint when category exists", () => {
    expect(buildProductsEndpoint({ limit: 12, skip: 0, category: "smartphones" })).toBe(
